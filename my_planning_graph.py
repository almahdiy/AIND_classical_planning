...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        layers.ActionNode
        """
        #print("actionA effects: ", self.children[actionA])
        #print("actionB effects: ", self.children[actionB])
        effectsA = self.children[actionA]
        negEffectsB = {~effect for effect in self.children[actionB]}

        for effect in effectsA:
            if(effect in negEffectsB):
                return True
        return False
        #print("Self.children: {}".format(self.children))
        #print("actionA effects: ", actionA.effects)
    ...
    def _interference(self, actionA, actionB):
        ...
        negPrecondA = {~precond for precond in self.parents[actionA]}
        negPrecondB = {~precond for precond in self.parents[actionB]}

        for effect in effectsA:
            if(effect in negPrecondB):
                return True

        for effect in effectsB:
            if(effect in negPrecondA):
                return True
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        for condA in precondsA:
            for condB in precondsB:
                #print("Pair: {}, {}".format(condA, condB))
                #self.parent_layer is a Literal Layer:
                #When printed: <my_planning_graph.LiteralLayer object at 0x7f97294f1cf8>
                #found a function in the examples file
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        #print("Actions leading to literal A: ", actionsA)
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        #literals_lists = [list(layer.children.keys()) for layer in self.literal_layers]
        #all_literals = [literal for l in literals_lists for literal in l] 
    ...
    def h_maxlevel(self):
        ...
        self.fill() #fill the planning graph until it levels off
        #literals_lists = [list(layer.children.keys()) for layer in self.literal_layers]
        #all_literals = {literal for l in literals_lists for literal in l} #using set to eliminate redundancies and reduce runtime.
